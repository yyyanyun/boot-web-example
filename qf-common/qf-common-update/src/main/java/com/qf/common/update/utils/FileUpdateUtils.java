...
public class FileUpdateUtils {
    ...
    public  String load(MultipartFile file) {
        try {
            ...
            // 获取 文件上传对象
            PutObjectRequest request = new PutObjectRequest(aliYunOssProperties.getBucketName(), objectName,new ByteArrayInputStream(file.getBytes()));
    ...
    public    String getImagUrl(String objectName) {
        ...
        URL url = oss.generatePresignedUrl(aliYunOssProperties.getBucketName(), objectName, expiration);
        return url.toString();
    }
}
