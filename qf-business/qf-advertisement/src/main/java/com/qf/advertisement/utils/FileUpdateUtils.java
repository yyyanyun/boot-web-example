...
import com.qf.advertisement.config.OssConfigProperties;
...
public class FileUpdateUtils {
    ...
    public static String getKey(String originalFilename) {
        ...
        String fileName = String.format("%s%s%s%s", VIDEO_PREFIX_PATH, currentDate, number, suffixName);

        return String.format("%s%s%s%s", VIDEO_PREFIX_PATH, DateUtils.formatDate(new Date(), "yyyyMMdd"), "/", fileName);
    }
